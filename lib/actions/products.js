...
// Los mismos dos valores que acepta la constraint products_badge_check.
...
function readBasicFields(formData) {
  ...
  // ---------- Oferta y etiqueta (cambio #12) ----------
  // El campo vacío llega como "" y tiene que guardarse como null, no como 0:
  // un 0 pasaría el check de la base al revés y dibujaría "$0" tachado.
  const rawCompare = String(formData.get("compare_at_price") ?? "").trim();
  ...
  if (rawCompare) {
    ...
    if (!Number.isFinite(parsed) || parsed <= 0) {
      return { error: "El precio anterior tiene que ser un número válido." };
    }
  ...
  const rawBadge = String(formData.get("badge") ?? "").trim();
  const badge = rawBadge || null;

  if (badge && !BADGE_VALUES.includes(badge)) {
    return { error: "Esa etiqueta no existe." };
  }

  // Sin precio anterior, "Oportunidad" queda como un cartel sin fundamento:
  // la clienta ve la etiqueta de oferta y el mismo precio de siempre.
  if (badge === "oportunidad" && compareAtPrice === null) {
    return {
      error:
        "Para marcar el producto como Oportunidad cargá también el precio anterior.",
    };
  }
